...
# Удалить с сервера файлы, которые site/ когда-то содержал, а теперь нет (чужие не трогает).
tools/stale-files.sh | ssh -i "$DEPLOY_KEY" "$DEPLOY_HOST" "cd '$DEPLOY_PATH' && xargs -r -d '\n' rm -f --"
